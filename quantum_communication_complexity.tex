...
\begin{document}
    ...
    \section{Relations}
        ...
        $\alpha$-matching is a promise function related to the hidden matching problem described earlier. It was first studied for one-way communication complexity by Gavinsky et al.~\cite{Gavinsky:2007:ESO:1250790.1250866}. In $\alpha$-matching, the matching given to Bob consists of $\alpha n$ pairs, such that no index occurs in more than one pair. If $\alpha = 1/2$ then the matching is a perfect matching like before, and if $\alpha < 1/2$ then the matching is called a $partial$ matching. The problem and its quantum protocol are described as follows, where $\mathcal{M}_{\alpha n}$ is the set of all $\alpha$-matchings:
        ...
        \begin{codebox}
            ...
            \li Bob outputs $0$ if $|\psi_{i\pm j}\rangle = \frac{\pm 1}{\sqrt{2}}(|i\rangle+|j\rangle)$ and $1$ if $|\psi_{i\pm j}\rangle = \frac{\pm 1}{\sqrt{2}}(|i\rangle-|j\rangle)$.
        \end{codebox}
